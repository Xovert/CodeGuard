...
from flask import current_app as app, redirect, send_file
...
class FilesystemUploads(Uploads):
    def __init__(self, base_path=None):
        super(Uploads, self).__init__()
        self.base_path = base_path or app.config.get("UPLOAD_FOLDER")
    ...
    def upload(self, file_obj, filename, path=None):
        ...
        if path:
            path = secure_filename(path) or os.urandom(16).hex()
            path = path.replace(".", "")
        else:
            path = os.urandom(16).hex()
...
class S3Uploads(Uploads):
    def __init__(self):
        super(Uploads, self).__init__()
        self.s3 = self._get_s3_connection()
        self.bucket = app.config.get("AWS_S3_BUCKET")
        # If the custom prefix is provided, add a slash if it's missing
        custom_prefix = app.config.get("AWS_S3_CUSTOM_PREFIX")
        ...
        try:
            ...
            # For MinIO, this is typically 'NoSuchBucket' or '404'
            if error_code in ('NoSuchBucket', '404'):
                print(f"Bucket '{self.bucket}' does not exist in MinIO. Creating it...")
                # Create the bucket (MinIO doesn't typically need a LocationConstraint)
                self.s3.create_bucket(Bucket=self.bucket)
            else:
                # Some other error (e.g., permission denied or invalid bucket name)
                raise
    ...
    def _get_s3_connection(self):
        ...
        endpoint = app.config.get("AWS_S3_ENDPOINT_URL")
        region = app.config.get("AWS_S3_REGION")
        client = boto3.client(
            "s3",
    ...
    def download(self, filename):
        ...
        with freeze_time(datetime.datetime.fromtimestamp(truncated_timestamp, tz=datetime.timezone.utc)):
        ...
        custom_domain = app.config.get("AWS_S3_CUSTOM_DOMAIN")
        if custom_domain:
    ...
    def sync(self):
        local_folder = app.config.get("UPLOAD_FOLDER")
